function tvRemote(word) {
  var keyBoard = {'a':[0, 0], 'b':[0, 1], 'c':[0, 2], 'd':[0, 3], 'e':[0, 4], '1':[0, 5], '2':[0, 6], '3':[0, 7],
  ...
  var clicks = 0;
  word.split('').map(letter => keyBoard[letter]).reduce((r, c) => {
    clicks += Math.abs(r[0] - c[0]) + Math.abs(r[1] - c[1]) + 1;
    return c;
  },[0, 0]);
  return clicks;
}
